...
export const SecondNavbar = ({ navItems }: iSecondNavbarProps) => {
  ...
  return (
    <>
      <div
        className={`bg-[#F8F9FB] flex dark:bg-[#18181b] dark:border-r dark:border-r-[#ffffff1a]`}
      >
        {isExpand.app && (
          <div className="w-64 flex-1 py-[16px] pl-[16px]">
            <h1 className="text-black font-semibold text-lg mb-[20px] dark:text-white">
              {(navItems[0].items.find((item) => item.route === currentUrl)
                ?.itemCategory || "Matrix Apps") + " Controls"}
            </h1>
            <NavigationBar
