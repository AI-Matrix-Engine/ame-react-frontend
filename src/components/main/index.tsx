    ...
    <div className="flex h-screen flex-col">
      ...
      <div className="flex relative h-full  overflow-x-hidden overflow-y-hidden    ">
        {sideBars.map((sideBar: leftSideBarType) => (
          (pathname !== "/register" && pathname !== "/login") && <div
            id="scrollableDiv"
        ...
        {sideBars[1].toggle && (
          <SecondSidebarToggleButton
            ...
            onHandle={onExpand}
            handleMouseEvent={(toggle: any) => handleMouseEventButton(toggle)}
            setOpacity={(value: any) => setOpacity(value)}
            opacity={opacity}
          />
        )}
        <div className={`flex-1 overflow-y-auto `}>{children}</div>
        {(pathname !== "/register" && pathname !== "/login") && <RightNavbar />}
      </div>
    </div>
  );
};
