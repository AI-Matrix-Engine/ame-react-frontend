...
export const fetchTotalData = async (endpoint: string): Promise<any[]> => {
  try {
    const response = await axios.get(
      `${aiMatrixAPI.defaults.baseURL}${endpoint}`
    );
...
export const fetchOneDataById = async (
  endpoint: string,
  id: number
): Promise<any> => {
  try {
    const response = await axios.get(
      `${aiMatrixAPI.defaults.baseURL}${endpoint}/${id}/`
    );
...
export const partialUpdateApiById = async (
  endpoint: string,
  id: number,
  param: any
): Promise<void> => {
