...
export const fetchCategories = async (): Promise<iSelectType[]> => {
  try {
    const response = await axios.get(
      `${aiMatrixAPI.defaults.baseURL}oai/allcategory/`
    );
    const formattedCategories: iSelectType[] = response.data.reduce(
      (acc: iSelectType[], item: any) => {
        if (item.parent_category === null) {
          ...
          const parentIndex = acc.findIndex(
            (cat) => cat.value === item.parent_category.toString()
          );
          if (parentIndex > -1) {
