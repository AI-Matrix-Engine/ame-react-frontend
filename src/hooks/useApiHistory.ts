...
import { fetchTotalData, createData, fetchOneDataById, updateDataById, deleteDataById } from "@/services/integrateService";
...
export const useApiHistory = () => {
    ...
    const getApiHistories = async () => {
        const fetchedHistories: iApiCallHistoryType[] = await fetchTotalData(subAiAPI.apiCallHistory);
    ...
    const handleFetchHistoryById = async (id: number) => {
        const result: iApiCallHistoryType | null = await fetchOneDataById(subAiAPI.apiCallHistory, id);
        FetchHistory(result);
    }
    ...
    const handleUpdateHistoryById = async (id: number, historyData: iApiCallHistoryInputType) => {
